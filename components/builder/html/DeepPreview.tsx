...
export default function DeepPreview({
  ...
  useEffect(() => {
    ...
    if (finalHtml.includes("</body>")) {
      finalHtml = finalHtml.replace(
        "</body>",
        `<script id="crawlcube-editor-script">${EDITOR_SCRIPT}</script></body>`,
      );
    } else {
    ...
    // After setting src, wait for the iframe to load then sync the editable state.
    // This ensures the editor script's message listener is ready before we send.
    const onLoad = () => {
      iframe.contentWindow?.postMessage(
        { type: "SET_EDITABLE", data: editableRef.current },
        "*",
      );
    };
  ...
  // Keep editableRef in sync so the onLoad handler reads the latest value
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (iframeRef.current && iframeRef.current.contentWindow) {
      iframeRef.current.contentWindow.postMessage(
        { type: "SET_EDITABLE", data: editable },
        "*",
      );
    }
  }, [editable]);
