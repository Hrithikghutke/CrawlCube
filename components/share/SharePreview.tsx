...
import ReactSandpack from "@/components/builder/react/ReactSandpack";
...
export default function SharePreview({ shareId }: { shareId: string }) {
  ...
  const [reactFiles, setReactFiles] = useState<any>(null);
    ...
    const fetchPreview = async () => {
      try {
        ...
        if (gen.reactFiles) {
          ...
          setReactFiles(gen.reactFiles);
        } else if (gen.deepHtml) {
  ...
  if (notFound || (!layout && !deepHtml && !reactFiles)) {
    return (
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      {/* Preview */}
      {/* Preview */}
      <div className="flex-1 overflow-hidden bg-neutral-800 flex flex-col items-center p-0 md:p-4">
        {reactFiles ? (
          <ReactSandpack
            files={reactFiles}
            viewMode="preview"
          ...
          <div
            className="bg-background @container rounded-lg shadow-2xl transition-all duration-300 origin-top overflow-x-hidden overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 scrollbar-track-transparent"
            style={{
